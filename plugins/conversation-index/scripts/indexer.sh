...
function indexConversation(db, conversationPath, projectPath, encodedPath) {
  ...
  const state = db.prepare('SELECT last_indexed_line, last_modified_time FROM index_state WHERE conversation_uuid = ?').get(uuid);
  ...
  if (state && lines.length < state.last_indexed_line) {
    ...
    db.prepare('DELETE FROM index_state WHERE conversation_uuid = ?').run(uuid);
    db.prepare('UPDATE conversations SET message_count = 0 WHERE uuid = ?').run(uuid);
  ...
  for (let i = startLine; i < lines.length; i++) {
    ...
    if (extracted) {
      for (const item of extracted) {
        ...
        insertMessage.run(uuid, timestamp, item.role, content);
        indexed++;
      }
    }
  }
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?)
  `).run(uuid, lines.length, lastModified);

  // Update message count
  db.prepare('UPDATE conversations SET message_count = message_count + ? WHERE uuid = ?').run(indexed, uuid);

  return { indexed, skipped: false };
}

// Main indexing function
function indexAllConversations(fullReindex = false) {
  const db = initDB();
...
indexAllConversations(fullReindex);
