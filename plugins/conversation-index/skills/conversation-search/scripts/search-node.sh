...
async function search(query, scope = 'current_project', currentProject = null, limit = 100) {
  ...
  const summaries = results.map(result => {
    const msgStmt = db.prepare(`
      ...
      LIMIT 1
    `);
    msgStmt.bind([result.uuid]);
    const firstMessage = msgStmt.step() ? msgStmt.getAsObject() : null;
    msgStmt.free();
...
`);
  ...
  const results = await search(query, scope, currentProject, limit);
